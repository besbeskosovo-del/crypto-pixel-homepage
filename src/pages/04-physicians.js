...
const refer = {
  ...
  body: [
    `<section class="hero" aria-label="Introduction"><div class="inner">
      ...
      <p class="lead lh-body-lg">Little Hearts Home Care takes Georgia Medicaid members under 21 who need skilled nursing at home: ventilator, tracheostomy, enteral feeding, complex airway, seizure and complex medication cases. Every referral is read by an owner. [Owner 1 name], [credential], takes clinical calls directly at [phone].</p>
    ...
    H.section(
      `<h2 class="lh-h2">What happens next</h2><div class="mt-6">${H.stepGrid([
        ['Same day', 'An owner calls you and the family. [CONFIRM]'],
        ['Assessment', 'Within [CONFIRM] days: an RN completes a home or bedside assessment and drafts the plan of care.'],
    ...
    H.section(
      ...
      <p class="form-note mt-5">Received referrals: an owner will call you [CONFIRM: within one business day]. For a discharge this week, call ${S.MAIN_PHONE} instead of waiting.</p>
      <p class="after-grid">${H.link('Our clinical model', '/physicians/clinical-model')} &nbsp; ${H.link('Conditions we serve', '/physicians/conditions')} &nbsp; ${H.link('Download the referral form (PDF) [CONFIRM: build a one page fillable PDF]', '#form')}</p>`,
      { label: 'Referral form' }),
  ].join('\n'),
};
...
const clinicalModel = {
  ...
  body: [
    `<section class="hero" aria-label="Introduction"><div class="inner">
      ...
      <p class="lead lh-body-lg">We keep our caseload at what three owners can supervise personally. Here is how a case runs from your order to your next visit.</p>
    </div></section>`,
    H.section(
      `<div class="split">
        <div><h2 class="lh-h2">Staffing</h2><div class="mt-4">${H.checkList([
          ...
          'Trach and vent cases staffed only by nurses who have shown trach and vent competency [CONFIRM]',
        ], true)}</div></div>
        <div><h2 class="lh-h2">Supervision</h2><div class="mt-4">${H.checkList([
          'An RN supervisor on every case: [Owner or RN supervisor name], [credential]',
          'Initial RN assessment in the home, plan of care built from your orders',
          'Supervisory visits every [CONFIRM] days and after any change in condition',
          'RN and an owner on call [CONFIRM hours]',
    ...
    H.section(
      `<h2 class="lh-h2">Documentation</h2><div class="mt-4">${H.checkList([
        'Charting in [CONFIRM: EHR or paper] every shift: vitals, feeds, meds, vent settings, interventions, escalations',
        'Medication reconciliation at every order change and every hospital discharge',
        'A summary to you [CONFIRM: monthly, or on request] and at recertification',
        'Plan of treatment renewals sent to you [CONFIRM] days before expiration',
      ])}</div>`,
      { label: 'Documentation' }),
    H.section(
      `<div class="lh-card lh-card--flat"><h2 class="lh-h3">When something changes</h2>
      <p class="lh-body mt-3 measure">The nurse follows your parameters in the plan of care. Outside them, the nurse calls the RN supervisor, then your office or the on call line, and 911 when the child needs it. The family is told every time. You get a written note within [CONFIRM] hours.</p></div>`,
      { white: true, tight: true, label: 'Escalation' }),
    H.section(
      `<h2 class="lh-h2">Licensing and compliance</h2>
      <p class="lh-body mt-4 measure">Georgia Department of Community Health private home care provider license [NUMBER]. Enrolled GAPP provider [CONFIRM]. HIPAA compliant. [CONFIRM: accreditation, liability coverage statement]</p>
      ${related}`,
...
const conditionCards = [
  ...
  ['Nutrition', 'NG, G, GJ and J tube feeding, continuous and bolus schedules, TPN [CONFIRM].'],
  ...
  body: [
    ...
    H.section(
      `<div class="split">
        ...
        <p class="lh-body mt-3">Home infusion services beyond what our nurses can administer within the plan of care [CONFIRM], inpatient level monitoring, behavioral health cases without a skilled nursing need, and adults over 21. We’ll give you a name to call.</p></div>
        <div><h2 class="lh-h3">Ages</h2>
        <p class="lh-body mt-3">Birth to 21. GAPP ends at 21, and we start transition planning with the family and you a year before. [CONFIRM]</p></div>
      </div>
      ${related}`,
      { label: 'Scope' }),
  ].join('\n'),
